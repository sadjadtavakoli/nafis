...
class DatePickerModal extends React.Component {
  ...
  render() {
    return (
      ...
      <Modal dimmer={"blurring"} open={this.props.isOpen} onClose={this.props.onClose} size="mini">
          ...
          <Modal.Actions>
            <Button color='black' onClick={() => {this.props.onClose(); this.setState(INITIAL_STATE)}}>
              <p>لغو</p>
            </Button>
            <Button positive icon='checkmark' disabled={this.state.selectedDayRange.from == null || this.state.selectedDayRange.to == null} onClick={this.handleClickSubmit}>
              <p>تایید</p>
            </Button>
          </Modal.Actions>
        </Modal>
