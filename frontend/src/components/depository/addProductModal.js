...
class AddBillModal extends React.Component {
    ...
    this.setState(
      ...
      () => {
        if (inputName === "buying_price" || inputName === "selling_price") {
          this.setState({
            [inputName + "_persianLetter"]: Num2persian(
              Number(this.state[inputName])
            )
          });
        }
      }
  ...
  formSubmitHandler = () => {
    this.setState(
      { formValidation: { ...this.state.formValidation, ...FORM_VALIDATION } },
      () => {
        let hasError = false;
        if (String(this.state.code).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, code: true }
          });
          hasError = true;
        }
        if (String(this.state.name).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, name: true }
          });
          hasError = true;
        }
        if (String(this.state.buying_price).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, buying_price: true }
          });
          hasError = true;
        }
        ...
        if (String(this.state.stock_amount).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, stock_amount: true }
          });
          hasError = true;
        }
        // ---------- Options ----------
        ...
        if (String(this.state.design_color).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, design_color: true }
          });
          hasError = true;
        }
        if (String(this.state.material).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, material: true }
          });
          hasError = true;
        }
        if (String(this.state.f_type).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, f_type: true }
          });
          hasError = true;
        }
        if (String(this.state.design).length < 1) {
          this.setState({
            formValidation: { ...this.state.formValidation, design: true }
          });
          hasError = true;
        }
        if (!hasError) {
              ...
              <Form>
                <Form.Group widths={3}>
                  <Form.Select
                    ...
                    search
                    selection
                    error={this.state.formValidation.background_color}
                    onChange={this.selectChange}
                    name="background_color"
                    options={this.state.background_color_Options}
                  />
                  <Form.Select
                    fluid
                    placeholder="رنگ طرح"
                    ...
                    search
                    selection
                    error={this.state.formValidation.design_color}
                    onChange={this.selectChange}
                    name="design_color"
                    options={this.state.design_color_Options}
                  />
                  <Form.Select
                    fluid
                    placeholder="جنس"
                    ...
                    search
                    selection
                    error={this.state.formValidation.material}
                    onChange={this.selectChange}
                    name="material"
                ...
                <Form.Group widths={3}>
                  <Form.Select
                    ...
                    search
                    selection
                    error={this.state.formValidation.f_type}
                    onChange={this.selectChange}
                    name="f_type"
                    options={this.state.f_type_Options}
                  />
                  <Form.Select
                    fluid
                    placeholder="نوع طرح"
                    ...
                    search
                    selection
                    error={this.state.formValidation.design}
                    onChange={this.selectChange}
                    name="design"
                    options={this.state.design_Options}
                  />
                </Form.Group>
                <Divider />
                ...
                <Form.Group unstackable widths={2}>
                  <Form.Input
                    ...
                    type="number"
                    error={this.state.formValidation.code}
                  ...
                  <Form.Input
                    className="ltr placeholder-rtl"
                    label="نام محصول"
                    error={this.state.formValidation.name}
                ...
                <Form.Group unstackable widths={2}>
                  <Form.Input
                    ...
                    type="number"
                    error={this.state.formValidation.stock_amount}
                    onChange={e => this.inputChange(e, "stock_amount")}
                    placeholder="مقدار باقی مانده"
                  />
                  <Form.Dropdown
                ...
                <Form.Group unstackable widths={2}>
                  <Form.Input
                    ...
                    label="قیمت فروش"
                    type="number"
                    error={this.state.formValidation.selling_price}
                    onChange={e => this.inputChange(e, "selling_price")}
                  ...
                  <Form.Input
                    ...
                    label="قیمت خرید"
                    type="number"
                    error={this.state.formValidation.buying_price}
                    onChange={e => this.inputChange(e, "buying_price")}
                ...
                <Grid columns="two" divided>
                  <Grid.Row>
                    <Grid.Column
                      className={
                        this.state.selling_price.length > 0 ? "" : "invisible"
                      }
                    >
                      <Label>
                        {this.state.selling_price_persianLetter}&nbsp;
                        <span>تومان</span>
                      </Label>
                    </Grid.Column>
                    <Grid.Column
                      className={
                        this.state.buying_price.length > 0 ? "" : "invisible"
                      }
                    >
                      <Label>
                        {this.state.buying_price_persianLetter}&nbsp;
                        <span>تومان</span>
                      </Label>
                    </Grid.Column>
                  </Grid.Row>
...
export default connect(mapStateToProps, { getProductFields, setNewProduct })(
  AddBillModal
);
